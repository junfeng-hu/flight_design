...
(function(window, undefined) {
					...
					for (var i = 0, elem; (elem = elems[i]) != null; i++) {
						...
						if (typeof elem === "string") {
							if (!rhtml.test(elem)) {
								...
								// Clear elements from DocumentFragment
								// (safeFragment or otherwise)
								// to avoid hoarding elements. Fixes #11356
								if (div) {
									div.parentNode.removeChild(div);

									// Guard against -1 index exceptions in
									// FF3.6
	...
	if (document.defaultView && document.defaultView.getComputedStyle) {
		getComputedStyle = function(elem, name) {
			...
			// WebKit uses "computed value (percentage if specified)" instead of
			// "used value" for margins
			// which is against the CSSOM draft spec:
